...
fun SearchBar(
    ...
    if (breakpoint >= Breakpoint.SM || fullWidth) {
        ...
        Row(
            ...
            FaMagnifyingGlass(
                size = IconSize.SM,
                modifier = Modifier.color(if (focused) Theme.Primary.rgb else Theme.DarkGray.rgb)
                    .margin(right = 14.px)
            )
            org.jetbrains.compose.web.dom.Input(type = InputType.Text, attrs = Modifier
                .fillMaxSize()
